...
export default function LegalChat() {
    ...
    const handleThreatResult = useCallback((result, sourceMessage) => {
        const summary = result.is_threat_detected
            ? `Threat detected. ${result.bns_section || ''} ${result.draft_fir_text || ''}`
            : 'No threat detected. If you feel unsafe, contact your local support group.'

        const userMsg = { id: `u-threat-${Date.now()}`, role: 'user', text: sourceMessage, time: now() }
        const aiMsg = { id: `a-threat-${Date.now() + 1}`, role: 'ai', text: summary.trim(), time: now() }
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden">
            ...
            <ThreatReportPanel uid={currentUserUid} onResult={handleThreatResult} />
