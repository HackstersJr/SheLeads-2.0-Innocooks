...
import { registerUser, sendOtp } from '../api/shevestApi';
...
export default function Login() {
  const { login, setApiNotice } = useApp();
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      setError(apiError.message || 'Could not send OTP right now. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const payload = {
        ...
        upi_vpa: `member${phone.slice(-4)}@upi`,
        device_id: `device_${phone.slice(-6)}`,
        org_id: 'org_demo_001',
      };
      const response = await registerUser(payload);
      login('borrower', response.data);
      setApiNotice('');
    } catch (apiError) {
  ...
  const handleQuickLogin = (role) => {
    // Use the known demo user so all backend calls (pay-installment, loans/market, etc.) work.
    login(role, { uid: 'user_demo_001', trust_score: 20, chit_cycles_completed: 0 })
  }
